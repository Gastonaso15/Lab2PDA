@echo off
REM Script para iniciar solo los servicios (Servidor Central + Tomcat, SIN Estación de Trabajo)
REM Uso: iniciar-servicios.bat
...
for /f "tokens=2" %%a in ('netstat -ano ^| findstr :9128 ^| findstr LISTENING') do (
    for /f "tokens=5" %%b in ('netstat -ano ^| findstr :9128 ^| findstr LISTENING') do (
        set "WS_PID=%%b"
        goto :ws_pid_found
    )
)
...
REM Los PIDs se guardarán en detener-servicios.bat
